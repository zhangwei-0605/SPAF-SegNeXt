...
class RSImagePredictManager(object):
    ...
    def __init__(
        ...
        self.geo_transform = self.in_raster.GetGeoTransform()
        self.projection = self.in_raster.GetProjection()

        proj = self.in_raster.GetProjection()
        geotrans = self.in_raster.GetGeoTransform()
    ...
    def generate_windows(self, net_sz, fixed):
        ...
        if (self.img_h == 5120 and self.img_w == 5120 and
                self.window_sz == 2816 and self.overlap == 512):
            windows = [
                [0, 2816, 0, 2816],
                [0, 2816, 2304, 5120],
                [2304, 5120, 0, 2816],
    ...
    def get_next(self):
        ...
        band4 = self.in_raster.GetRasterBand(4).ReadAsArray(x1, y1, x2 - x1, y2 - y1)
        band3 = self.in_raster.GetRasterBand(3).ReadAsArray(x1, y1, x2 - x1, y2 - y1)
        band2 = self.in_raster.GetRasterBand(2).ReadAsArray(x1, y1, x2 - x1, y2 - y1)

        band4_norm = self.std_stretch(band4, n_std=2)
        band3_norm = self.std_stretch(band3, n_std=2)
        band2_norm = self.std_stretch(band2, n_std=2)

        rgb_image = np.dstack((band4_norm, band3_norm, band2_norm))

        im_data = rgb_image.transpose(2, 0, 1)
        assert im_data.max() <= 256, im_data.max()
        if im_data.ndim == 2:
            im_data = im_data[np.newaxis, :, :]
        self.window_i += 1
        return im_data.astype(np.uint8), (y1, y2, x1, x2)
    def std_stretch(self,band, n_std=2):
        masked = band[band > 0] if np.any(band > 0) else band
        mean, std = masked.mean(), masked.std()

        lower = max(mean - n_std * std, band.min())
        upper = min(mean + n_std * std, band.max())

        band_norm = np.clip((band - lower) / (upper - lower) * 255, 0, 255)
    ...
    def update(self, result):
        ...
        is_top = (y1 == 0)
        is_bottom = (y2 == self.img_h)
        is_left = (x1 == 0)
        is_right = (x2 == self.img_w)

        dy1 = 0 if is_top else overlap_half
        dy2 = 0 if is_bottom else overlap_half
        dx1 = 0 if is_left else overlap_half
        dx2 = 0 if is_right else overlap_half

        crop_height = (y2 - y1) - dy1 - dy2
        crop_width = (x2 - x1) - dx1 - dx2

        if crop_height <= 0 or crop_width <= 0:
            print(f"错误：裁剪后尺寸无效 (h={crop_height}, w={crop_width})")
            return

        result_cropped = result[:, dy1:dy1 + crop_height, dx1:dx1 + crop_width]

        for i in range(self.out_bands):
            self.out_raster.GetRasterBand(i + 1).WriteArray(
                result_cropped[i],
                x1 + dx1,
                y1 + dy1
            )
