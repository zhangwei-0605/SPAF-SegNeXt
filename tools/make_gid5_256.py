...
def split_single_image(img_path, target_dir, split_size, overlap, val_rate):
    ...
    img_id = img_name.rstrip('.jpg')
    print(f'> {img_path}')

    stride = split_size - overlap
    img_path = os.path.join(input_dir, img_name)
    label_path = os.path.join(input_dir.rstrip('image_RGB') + 'label_5classes', img_id + '_5label.tif')
...
if __name__ == '__main__':
    src_dir = 'E:\dataset_root'
    dst_dir = 'E:\dataset_root\GID_5_512'
    split_size = 512
    overlap = 100
    val_rate = 0.1
    ...
    imgs = sorted([osp.join(src_dir, 'image_RGB', f) for f in os.listdir(osp.join(src_dir, 'image_RGB')) if f.endswith('.jpg')])
