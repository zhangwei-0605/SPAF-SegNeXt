...
class DualDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        ds = gdal.Open(data[0])
        if ds is None:
            raise ValueError("无法打开影像文件，请检查路径！")

        band4 = ds.GetRasterBand(4).ReadAsArray()
        band3 = ds.GetRasterBand(3).ReadAsArray()
        band2 = ds.GetRasterBand(2).ReadAsArray()

        band4_norm = std_stretch(band4, n_std=2)
        band3_norm = std_stretch(band3, n_std=2)
        band2_norm = std_stretch(band2, n_std=2)

        rgb_image = np.dstack((band4_norm, band3_norm, band2_norm))

        if rgb_image is None:
            raise ValueError(f"转换RGB失败: {data[0]}")
